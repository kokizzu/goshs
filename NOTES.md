...
- [DONE] The TUI in general is just one line as header and one line for category and then a growing space of lines. It would look nicer to reserve a reasonable amount of space that will grow and incorporate a scrollbar, if running out of space maybe. You could reserve the space of the windows available. → body now fills the full window height (banner/tabs above, status bar below) and the list/detail panes gain a vertical scrollbar in the rightmost column when content overflows.
...
- [DONE] I would love to have the name goshs be represented in a form of a banner at the top of the TUI somehow to make it look stylish and then move the information from the top to a real status bar at the bottom with the instructions on how to use (switch, scroll, detail, ...) → goshs figlet banner (Nord cyan) at top with version; controls moved into a context-sensitive status bar at the bottom (separate hints for list vs detail view).
- [DONE] In the status bar we could add more information like the version of goshs for example or the directory we are serving from. Maybe look at httpserver/info.go. Some of the information I would consider are: goshsversion, directory, ip/port/url, webdav, tunnel-url, cli enabled?, upload-folder if different from serve folder, auth user, process user, upload only, read only, no delete, shared links, dns port, smtp port, smb port and domain and share, ldap port, ttl-timer → status bar shows version, url, dir, upload-folder (if different), ttl, auth user, process user, read-only/upload-only/no-delete, webdav, cli, tunnel, dns/smtp/smb(+domain\share)/ldap ports, catcher. Caveat: the live tunnel-URL and shared-link count live on the FileServer at runtime, not options.Options, so they are not surfaced here (tunnel shown as a flag only).
- [DONE] We could make more use of lipgloss to make it look a little nicer. Maybe incorporate nord theme again, as in the webUI and use emojis or icons for the tabs and the controls → Nord palette applied throughout (banner, tabs, selection, status bar, scrollbar, per-protocol event accents); tabs carry per-protocol icons and the controls line uses arrow/symbol glyphs.
...
- [DONE] While in details view, for example looking at http collab events, if a new event comes in the view updates. It would be better to stick to the details view regardless if there are any new → selection now anchors to the viewed event; new events only ride the top in the list view when already at the top.
  - [DONE] After fixin the issue, now if a new record appears while in detail view, it will not be shown unless the users hits the up arrow. This is unfortunate. Is there a better way of doing it? → detail header now shows a live position badge with a "↑ N newer" count that ticks up as events arrive while the inspected event stays anchored; ↑/↓ step between events and home/g (end/G) jump to the newest/oldest. Long detail bodies are scrollable with PgUp/PgDn.
